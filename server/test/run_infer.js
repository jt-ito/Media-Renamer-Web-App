...
// Minimal JS emulation of server/src/parse.ts -> inferFromPath so this test
// can run without TypeScript tooling. This duplicates parsing logic but is
...
function stripTrailingRangeAndComplete(s) {
	let out = s.trim();
...
function inferFromPath(fullPath) {
	...
	if (multi && multi[1]) {
		...
		if (sxxexx) {
			...
			const absNums = range
				...
				: (baseClean.match(/\b(\d{1,3})\b/g) || []).map(Number).filter(n => n <= 300);
