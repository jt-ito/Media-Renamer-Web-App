...
const ring = [];
const RING_MAX = 2000;
let runtimeLevel = (initialLevel === 'debug' ? 'debug' : (initialLevel === 'warn' ? 'warn' : (initialLevel === 'error' ? 'error' : 'info')));
...
process.on('uncaughtException', (err) => {
    try {
        ring.push({ level: 'error', msg: `uncaughtException: ${String(err)}`, time: Date.now() });
    }
    catch { }
    try {
...
process.on('unhandledRejection', (r) => {
    try {
        ring.push({ level: 'error', msg: `unhandledRejection: ${String(r)}`, time: Date.now() });
    }
    catch { }
    try {
...
try {
    ...
    console.log = (...args) => {
        try {
            ring.push({ level: 'info', msg: args.map(a => String(a)).join(' '), time: Date.now() });
            if (ring.length > RING_MAX)
                ring.splice(0, ring.length - RING_MAX);
        }
        catch (e) { }
    ...
    console.error = (...args) => {
        try {
            ring.push({ level: 'error', msg: args.map(a => String(a)).join(' '), time: Date.now() });
            if (ring.length > RING_MAX)
                ring.splice(0, ring.length - RING_MAX);
        }
        catch (e) { }
    ...
    console.warn = (...args) => {
        try {
            ring.push({ level: 'warn', msg: args.map(a => String(a)).join(' '), time: Date.now() });
            if (ring.length > RING_MAX)
                ring.splice(0, ring.length - RING_MAX);
        }
        catch (e) { }
    ...
    console.debug = (...args) => {
        try {
            ring.push({ level: 'debug', msg: args.map(a => String(a)).join(' '), time: Date.now() });
            if (ring.length > RING_MAX)
                ring.splice(0, ring.length - RING_MAX);
        }
        catch (e) { }
