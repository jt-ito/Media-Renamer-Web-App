...
export function inferFromPath(fullPath) {
    ...
    if (epRange?.groups && epRange.groups.e1 && epRange.groups.e2) {
        ...
        if (!isNaN(start) && !isNaN(end) && end >= start) {
            ...
            if (sxx?.groups && sxx.groups.s)
                season = Number(sxx.groups.s);
            confidence += 3;
        }
    }
    if (multi?.groups?.s) {
    ...
    let episode_title = undefined;
    let parsedNameOut = undefined;
    let jellyfinExampleOut = undefined;
    try {
    ...
    try {
        log('debug', `inferFromPath: ${fullPath} -> ${JSON.stringify(result)}`);
    }
    catch { }
    return result;
}
