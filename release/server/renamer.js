...
function applyScheme(template, vars) {
    try {
        ...
        for (let i = 0; i < template.length; i++) {
            if (template[i] === '{')
                ...
                open--;
            if (open < 0)
                throw new Error('Template syntax error: unexpected }');
        }
        if (open !== 0)
            throw new Error('Template syntax error: missing } in template expression');
...
export function movieOutputPath(lib, cand, ext) {
...
export function episodeOutputPath(lib, series, season, eps, episodeTitle, ext) {
    // Normalize ext: ensure it's a string and, if present, starts with a dot.
    ext = ext ? String(ext) : '';
    if (ext && !ext.startsWith('.'))
        ext = '.' + ext;
    ...
    if (!year && lib && lib.inputRoot) {
        ...
        if (m) {
            year = m[0];
            debugSource = 'inputRoot-any-4digit';
        }
    }
...
export function planMovie(item, lib, cand) {
    const to = movieOutputPath(lib, cand, item.ext);
    try {
        log('debug', `planMovie: ${item.path} -> ${to} (tvdb=${cand.id})`);
    }
    catch { }
...
export function planEpisode(item, lib, series, season, eps, episodeTitle) {
    const to = episodeOutputPath(lib, series, season, eps, episodeTitle, item.ext);
    try {
        log('debug', `planEpisode: ${item.path} -> ${to} (series=${series.id} s=${season} e=${eps.join(',')})`);
    }
    catch { }
    const metadataTitle = path.basename(to, path.extname(to));
    ...
    const plan = { from: item.path, to, action: lib.linkMode || 'hardlink', dryRun: true, meta: { tvdbId: series.id, type: 'series', output: to, metadataTitle, year: metaYear } };
    try {
...
export function applyPlans(plans, allowCopyFallback) {
    const results = [];
    const journal = [];
    try {
        for (const p of plans) {
            try {
                log('info', `applyPlans - processing: ${p.from} -> ${p.to} (action=${p.action})`);
            }
            catch { }
            if (p.dryRun)
                continue;
            ...
            try {
                ...
                if (p.meta) {
                    ...
                    try {
                        p.meta.metadataTitle = path.basename(target, path.extname(target));
                    }
                    catch { }
            ...
            if (p.action === 'hardlink') {
                try {
                    ...
                    if (e.code === 'EXDEV' && allowCopyFallback) {
                        ...
                        try {
                            log('error', `link failed for ${p.from} -> ${target}: ${e?.message ?? String(e)}`);
                        }
                        catch { }
...
export async function finalizePlan(p) {
    try {
